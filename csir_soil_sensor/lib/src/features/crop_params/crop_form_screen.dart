...
class _CropFormScreenState extends ConsumerState<CropFormScreen> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            wasEditing ? 'Crop parameters updated' : 'Crop parameters saved',
          ),
        ),
      );
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(title: const Text('Crop Parameters')),
  ...
  Future<void> _confirmDelete(CropParam item) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('This will delete Crop set #${item.id} and its images.'),
        actions: [
          TextButton(
